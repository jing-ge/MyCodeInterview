...
const BinaryTreeNode* KthNodeCore(const BinaryTreeNode* pRoot, unsigned int& k)
{
    const BinaryTreeNode* res = nullptr;
    if(pRoot->m_pLeft!=nullptr)
        res = KthNodeCore(pRoot->m_pLeft,k);
    if(res==nullptr){
        if(k==1)return pRoot;
        k--;
    }
    if(res==nullptr&&pRoot->m_pRight!=nullptr){
        res = KthNodeCore(pRoot->m_pRight,k);
    }
    return res;
}
...
int main(int argc, char* argv[])
{
    ...
    const BinaryTreeNode* res = KthNode(node1,1);
